...
**Prefiks tagu dla tego repozytorium:** `shipping/v` (np. `shipping/v1.2.3`)
...
2. Utwórz i wypchnij tag:
   ...
   git tag shipping/v1.2.3
   git push origin shipping/v1.2.3
   ```

3. Workflow `release.yml` automatycznie:
   - zbuduje projekt,
   - opublikuje paczkę `TBJ.Integrations.Shipping` na NuGet.org (Trusted Publishing) i GitHub Packages,
   - utworzy GitHub Release,
   - zamrozi kod w gałęzi `release/shipping/1.2.3`.
...
1. Utwórz gałąź z właściwej gałęzi release:
   ...
   ```bash
   git checkout -b hotfix/shipping-1.2.3 release/shipping/1.2.3
   ```

   Możesz też utworzyć ją z `main`, jeśli poprawka jest już tam wprowadzona.
2. Wprowadź poprawki.
3. Otwórz PR do gałęzi `release/shipping/1.2.3`.
4. Po zatwierdzeniu i merge wypchnij nowy tag:

   ```bash
   git tag shipping/v1.2.4
   git push origin shipping/v1.2.4
   ```
...
Publikacja na NuGet.org odbywa się przez Trusted Publishing (OIDC). Na NuGet.org dla paczki `TBJ.Integrations.Shipping` musi być skonfigurowana polityka zaufania dla repozytorium `tbudaj/TBJ.Integrations.Shipping` i workflow `release.yml`.
